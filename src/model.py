...
@dataclass
class ConfigModelo:
    tamanho_vocab: int = 2000
    d_model: int = 192
    n_camadas: int = 6
    n_cabecas: int = 6
    n_cabecas_kv: int = 2
    d_ff: int = 576
    comprimento_max: int = 256
    dropout: float = 0.1
    rope_theta: float = 10_000.0

    def validar(self) -> None:
        if self.tamanho_vocab <= 0:
            raise ValueError("tamanho_vocab deve ser positivo.")
        ...
        if (
            self.d_model // self.n_cabecas
        ) % 2 != 0:
            raise ValueError(
                "A dimensão de cada cabeça deve ser par para RoPE."
            )
        if self.comprimento_max <= 0:
            raise ValueError(
                "comprimento_max deve ser positivo."
            )
...
class AtencaoCausalGQA(nn.Module):
    def __init__(
        ...
        self.q = nn.Linear(
            cfg.d_model,
            cfg.n_cabecas * self.d_cabeca,
            bias=False,
        )
        self.k = nn.Linear(
            cfg.d_model,
            cfg.n_cabecas_kv * self.d_cabeca,
            bias=False,
        )
        self.v = nn.Linear(
            cfg.d_model,
...
class BlocoTransformer(nn.Module):
    def __init__(
        ...
        self.norma_atencao = RMSNorm(
            cfg.d_model
        )
        self.atencao = AtencaoCausalGQA(
            cfg
        )
        self.norma_ffn = RMSNorm(
            cfg.d_model
        )
        self.ffn = SwiGLU(cfg)
...
class MiniLLM(nn.Module):
    def __init__(
        ...
        self.blocos = nn.ModuleList(
            [
                BlocoTransformer(cfg)
                for _ in range(cfg.n_camadas)
            ]
        )
        self.norma_final = RMSNorm(
            cfg.d_model
        )
    ...
    def forward(
        ...
        if ids.size(1) > self.cfg.comprimento_max:
            raise ValueError(
                f"Sequência com {ids.size(1)} tokens excede "
                f"o limite de {self.cfg.comprimento_max}."
            )
    ...
    def gerar(
        ...
        for _ in range(max_novos_tokens):
            ...
            if eos_id is not None:
                finalizadas = (
                    finalizadas
                    | proximo.squeeze(-1).eq(
                        eos_id
                    )
                )

                if bool(finalizadas.all()):
                    break
